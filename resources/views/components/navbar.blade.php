<header x-data="{ open: false, scrolled: false }" x-init="window.addEventListener('scroll', () => scrolled = window.scrollY > 20)" :class="scrolled
    ...
    <div class="mx-auto flex h-20 max-w-7xl items-center justify-between gap-6 px-5 sm:px-8 lg:px-10">
        ...
        {{-- Logo --}}
        <a href="/" class="group flex shrink-0 items-center gap-3" aria-label="الصفحة الرئيسية لـ MORA GYM">
        ...
        <nav class="hidden items-center gap-7 lg:flex">
            ...
            <a href="{{ route('member.attendance') }}" class="text-sm font-medium text-mora-muted transition hover:text-mora-accent">
                الحضور
            </a>
            ...
            <a href="{{ route('member.profile') }}" class="text-sm font-medium text-mora-muted transition hover:text-mora-accent">
                الملف الشخصي
            </a>
        ...
        <div class="hidden shrink-0 items-center gap-3 lg:flex">
            ...
            <form action="{{ route('logout') }}" method="POST">
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-md border border-mora-border px-5 py-2.5 text-sm font-semibold text-mora-text transition hover:border-mora-accent hover:text-mora-accent">
                    تسجيل الخروج
                </button>
            </form>
    ...
    <div x-show="open" x-cloak x-transition class="border-t border-mora-border bg-mora-bg lg:hidden">
        <nav class="mx-auto flex max-w-7xl flex-col px-5 py-6 sm:px-8">
            ...
            <a @click="open = false" href="#sessions" class="border-b border-mora-border py-4 text-sm font-medium text-mora-muted">
                مواعيد التدريب
            </a>

            <a @click="open = false" href="#contact" class="{{ auth()->check() ? 'border-b border-mora-border' : '' }} py-4 text-sm font-medium text-mora-muted">
                تواصل معنا
            </a>

            @auth

            <a @click="open = false" href="{{ route('member.attendance') }}" class="border-b border-mora-border py-4 text-sm font-medium text-mora-muted">
                الحضور
            </a>
            ...
            @if (($unreadNotificationsCount ?? 0) > 0)
            <span class="absolute -top-2 -left-2 min-w-5 h-5 px-1 rounded-full bg-mora-accent text-black text-xs font-bold flex items-center justify-center">
                {{ $unreadNotificationsCount > 99 ? '99+' : $unreadNotificationsCount }}
            </span>
            @endif
            ...
            <a @click="open = false" href="{{ route('member.profile') }}" class="py-4 text-sm font-medium text-mora-muted">
                الملف الشخصي
            </a>

            @if (auth()->user()->isAdmin())

            <a @click="open = false" href="{{ route('admin.dashboard') }}" class="mt-4 flex items-center justify-center gap-3 rounded-md bg-mora-accent px-5 py-4 text-xs font-bold tracking-wider text-mora-bg">
                لوحة الإدارة
            </a>

            @else

            <a @click="open = false" href="{{ route('member.dashboard') }}" class="mt-4 flex items-center justify-center gap-3 rounded-md bg-mora-accent px-5 py-4 text-xs font-bold tracking-wider text-mora-bg">
                لوحة التحكم
            </a>
            ...
            <form action="{{ route('logout') }}" method="POST" class="mt-3">
                ...
                <button type="submit" class="flex w-full items-center justify-center gap-3 rounded-md border border-mora-border px-5 py-4 text-xs font-bold tracking-wider text-mora-text">
                    تسجيل الخروج
                </button>
            </form>
            ...
            <a @click="open = false" href="{{ route('login') }}" class="mt-4 flex items-center justify-center gap-3 rounded-md bg-mora-accent px-5 py-4 text-xs font-bold tracking-wider text-mora-bg">
                تسجيل الدخول
            </a>
